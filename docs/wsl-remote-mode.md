...
HTTP on the loopback interface. WSL forwards `localhost` to Windows, so Quota
Arc asks `http://localhost:8787` for usage and draws the rings as usual.
...
|  127.0.0.1:8787              | localhost|  QUOTAARC_CODEXBAR_URL or    |
...
```powershell
setx QUOTAARC_CODEXBAR_URL http://localhost:8787
```
...
| `codexbarUrl` | CodexBar server URL | `http://localhost:8787` |
...
Set-ItemProperty -Path $key -Name codexbarUrl -Value "http://localhost:8787"
...
CodexBar also serves a dashboard at `http://localhost:8787/`. The page itself
...
- **Nothing answers.** Run `curl http://localhost:8787/health` in WSL, then
  `curl.exe http://localhost:8787/health` in PowerShell. If WSL answers and
...
- **Port 8787 is taken.** Choose another port for the service and point Quota
  ...
  ```powershell
  Set-ItemProperty -Path "HKCU:\Software\QuotaArc" -Name codexbarUrl -Value "http://localhost:8788"
  ```
